...
create view if not exists fwa_totals(name, total) as select us_totals.name, us_totals.total + arvn_totals.total from us_totals join arvn_totals on us_totals.name = arvn_totals.name;
create view if not exists non_nva_totals(name, total) as select fwa_totals.name, fwa_totals.total + vc_totals.total from fwa_totals join vc_totals on fwa_totals.name = vc_totals.name;
...
name, CASE
  WHEN us_troop + us_base + us_irregular + arvn_troop + arvn_base + arvn_ranger + arvn_police > nva_troop + nva_base + nva_tunnel_base + nva_guerrilla + vc_guerrilla + vc_base + vc_tunnel_base THEN 'COIN'
  WHEN nva_base + nva_tunnel_base + nva_guerrilla > us_base + us_irregular + arvn_troop + arvn_base + arvn_ranger + arvn_police + vc_guerrilla + vc_base + vc_tunnel_base THEN 'NVA'
  ELSE 'NONE'
  END
